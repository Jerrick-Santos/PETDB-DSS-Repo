...
function DeleteDiagnosticTest(props) {
  ...
  useEffect(() => {
    axios
      .get(`http://localhost:4000/api/checkdgtestref/${props.DGTestNo}`)
      .then((response) => {
  ...
  const handleDelete = async (e) => {
    ...
    try {
      await axios.delete(
        `http://localhost:4000/api/deletedgtest/${props.DGTestNo}`,
      );
    } catch (err) {
      console.log(err);
    }
  ...
  const handleStatusUpdate = async (e) => {
    ...
    try {
      await axios.patch(
        `http://localhost:4000/api/updatedgteststatus`,
        formValues,
      );
    } catch (err) {
      console.log(err);
        ...
        <Modal.Body>
          {isReferenced && props.isActive === 1 ? (
            <>
              ...
              Would you like to deactivate this diagnostic test?
              <Row className="mt-4">
                <Col>
                  <Button
...
export default DeleteDiagnosticTest;
