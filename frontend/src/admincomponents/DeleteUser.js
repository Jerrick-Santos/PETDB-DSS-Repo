...
function DeleteUser(props) {
  ...
  useEffect(() => {
    axios
      .get(`http://localhost:4000/api/checkuserref/${props.userNo}`)
      .then((response) => {
  ...
  const handleDelete = async (e) => {
    ...
    try {
      await axios.delete(
        `http://localhost:4000/api/deleteuser/${props.userNo}`,
      );
    } catch (err) {
      console.log(err);
    }
  ...
  const handleStatusUpdate = async (e) => {
    ...
    try {
      await axios.patch(
        `http://localhost:4000/api/updateuserstatus/${props.userNo}`,
        formValues,
      );
    } catch (err) {
      console.log(err);
    ...
    <>
      ...
      <Modal.Header style={{ color: "white", backgroundColor: props.isActive === 1 ? "#dc3545" : "#138313" }}>
        ...
        <Modal.Body>
          {isReferenced && props.isActive === 1 ? (
            <>
              <strong>{props.fullname} </strong>is currently being referenced in
            ...
            <>
              ...
              <strong> {props.fullname} </strong>? <br /> <br />
