...
const Treatments = () => {
  ...
  const [resistantData, setResistantData] = useState([]);
  const [caseData, setCaseData] = useState([]);
  const [isLoading, setIsLoading] = useState(true);
  useEffect(() => {
    axios
      .get(`http://localhost:4000/api/treatments/${caseNum}`)
      .then((res) => {
        console.log(res);
    ...
    axios
      .get(`http://localhost:4000/api/getCasePatient/${caseNum}`)
      .then((res) => {
        console.log(res);
        ...
        >
          {isLoading ? (
            ...
            <>
              <CaseHeader
                caseNo={caseNum}
                case_refno={caseData.case_refno}
                PatientNo={caseData.PatientNo}
                patient_name={caseData.patient_name}
                start_date={caseData.start_date}
                end_date={caseData.end_date}
                case_status={caseData.case_status}
                PRESref={caseData.PRESref}
                LATENTref={caseData.LATENTref}
              />
              ...
              <Row className="mt-5 justify-content-center">
                ...
                <Col lg="9">
                  ...
                  <hr />
                  <p style={{ fontSize: "25px" }}> Resistant Drugs </p>
                  {resistantData.length > 0 ? (
                    <Card className="mb-4">
                      <Card.Body>
                        <table className="table caption-top bg-white rounded mt-2 mb-0">
                          <thead>
                            <tr>
                              ...
                              <th scope="col">Refampicin (RIF)</th>
                              <th scope="col">Isoniazid (H)</th>
                              <th scope="col">Ethionamide (ETO)</th>
                              <th scope="col">Fluoroquinolones (FQ)</th>
                              <th scope="col">Amikacin (AMK)</th>
                            </tr>
                          </thead>
